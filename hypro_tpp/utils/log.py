...
class LogReader(object):
    ...
    def finished(self):
        return 'hypro_tpp finished' in self.doc
...
class LogBatchReader(object):
    ...
    def writeCSV(self):
        ...
        print(f"{c} out of {len(self.all_readers)} organized logs finished hypro_tpp")
...
def load_config(config_filename):
    ...
    # load data config spec
    with open(os.path.join(os.path.dirname(config_filename), 'data_spec.yaml')) as config_file:
        data_config_spec = yaml.safe_load(config_file)

    if 'data_name' in config['data']:
        #  add pad index information into model config
        data_name = config['data']['data_name']
        try:
            config['model'].update(data_config_spec[data_name])
        except KeyError:
            config['data'].update(data_config_spec[data_name])
