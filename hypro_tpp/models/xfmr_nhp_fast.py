...
class XFMRNHPFast(BaseModel):
    ...
    def forward(self, event_seqs, time_seqs, batch_non_pad_mask, attention_mask, extra_times=None, **kwargs):
    ...
    def compute_loglik(self, batch):
        ...
        enc_out = self.forward(event_seq[:, :-1], time_seq[:, :-1], batch_non_pad_mask[:, 1:],
    ...
    def _compute_intensities_fast(self,
        ...
        for i in range(0, num_samples, step):
            ...
            _attn_mask = attention_mask.unsqueeze(0).expand(_step, -1, -1, -1).reshape(_step * batch_size, seq_len,
                                                                                       seq_len)
            _enc_output = self.forward(_types,
                                       _times,
                                       _batch_non_pad_mask,
                                       _attn_mask,
                                       _extra_time)
    ...
    @staticmethod
    def soft_cross_entropy(label_proba, pred_proba):
        sum_ = - torch.sum(label_proba * torch.log(pred_proba) + (1 - label_proba) * torch.log(1 - pred_proba))
        return sum_

    def sim_score(self, state_1, state_2):
        state_1 = torch.tanh(state_1)  # values in [-1， 1]
        state_2 = torch.tanh(state_2)  # values in [-1， 1]
        hidden_size = state_1.size(-1)
        return torch.sum(state_1 * state_2, dim=-1) / ((hidden_size) ** (0.5))